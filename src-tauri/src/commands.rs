...
pub fn get_connectors(state: State<'_, AppState>) -> CmdResult<Vec<ConnectorRow>> {
    ...
    Ok(rows.into_iter().map(|r| {
        ...
        let extra = match service.as_str() {
            ...
            "lastfm" => serde_json::json!({ "albumListeners": state.real.scalar_i64("SELECT COUNT(*) FROM album_popularity WHERE found").unwrap_or(0), "taggedArtists": tag_of("lastfm"), "popularityArtists": state.real.scalar_i64("SELECT COUNT(*) FROM artist_popularity").unwrap_or(0) }),
