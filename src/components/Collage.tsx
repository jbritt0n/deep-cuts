...
export function Collage({ items, size = 3, className = '' }: { items: { id: string; title: string; subtitle?: string; imageUrl?: string | null }[]; size?: number; className?: string }) {
  ...
  return (
    <div className={`grid gap-1 overflow-hidden rounded-2xl ${className}`} style={{ gridTemplateColumns: `repeat(${size}, minmax(0, 1fr))` }} aria-label="Album art collage">
      {tiles.map((t) => t.imageUrl ? (
        <img key={t.id} src={t.imageUrl} alt={`${t.title}${t.subtitle ? ` — ${t.subtitle}` : ''}`} title={`${t.title}${t.subtitle ? ` — ${t.subtitle}` : ''}`} className="aspect-square w-full object-cover" loading="lazy" />
      ) : (
        <div key={t.id} title={`${t.title}${t.subtitle ? ` — ${t.subtitle}` : ''}`} className="grid aspect-square place-items-center p-2 text-center" style={{ background: `linear-gradient(135deg, hsl(${hue(t.id)} 22% 18%), hsl(${(hue(t.id) + 40) % 360} 28% 26%))` }}>
          <span className="font-display text-xs leading-tight text-cream/80 line-clamp-3">{t.title}</span>
        </div>
      ))}
    </div>
  );
