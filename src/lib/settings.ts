...
export const TUNING: Tunable[] = [
  { key: 'merge_isrc_versions', group: 'sessions', label: 'Merge versions of one recording', unit: '(1 = on)', min: 0, max: 1, step: 1, def: 1, rebuild: true, why: 'A single and its album cut often share an ISRC — the same recording released twice. On (1): their plays count as one song, under the version you played most; album statistics still credit the album each play came from. Off (0): every release counts separately.' },
  ...
  { key: 'thread_max_scene', group: 'threads', label: 'Scene-family threads at most', unit: '', min: 0, max: 30, step: 1, def: 8, why: 'Broad scene threads (West African, Psych…) can fill the chart; this caps them so niche tag threads keep their places.' },
  { key: 'thread_max_decade', group: 'threads', label: 'Decade threads at most', unit: '', min: 0, max: 12, step: 1, def: 4, why: 'Threads like "1970s" — how many the chart may show.' },
