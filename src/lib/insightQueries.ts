...
/** Phase 9c: one ISO week (Monday start), keyed by its Monday. */
export const periodForWeek = (monday: string): Period => { const d = new Date(monday + 'T00:00:00'); const dow = (d.getDay() + 6) % 7; d.setDate(d.getDate() - dow); const iso = (x: Date) => `${x.getFullYear()}-${String(x.getMonth() + 1).padStart(2, '0')}-${String(x.getDate()).padStart(2, '0')}`; const to = new Date(d.getTime() + 7 * 86400e3); const end = new Date(d.getTime() + 6 * 86400e3); const fmt = (x: Date) => x.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }); return { from: iso(d), to: iso(to), label: `Week of ${fmt(d)} – ${fmt(end)}, ${end.getFullYear()}` }; };
export const shiftWeek = (monday: string, n: number) => { const d = new Date(monday + 'T00:00:00'); d.setDate(d.getDate() + 7 * n); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`; };
