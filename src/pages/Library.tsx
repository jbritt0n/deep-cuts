...
function Playlists() {
  ...
  const T = totals.data;
  const SORTS: [PlaylistSort, string][] = [['affinity', 'Best fit for me'], ['least_affinity', 'Worst fit for me'], ['most_played', 'Most played'], ['least_played', 'Least played'], ['fewest_heard', 'Fewest songs heard'], ['most_complete', 'Most complete'], ['most_gems', 'Most hidden gems'], ['most_dead', 'Most dead weight'], ['stalest', 'Longest untouched'], ['biggest', 'Biggest'], ['newest', 'Recently added to']];
    ...
    <div>
      {T && (
        <div className="mb-4 flex flex-wrap items-center gap-x-6 gap-y-2 rounded-xl border border-line bg-ink/40 px-4 py-3 text-xs text-dust">
          ...
          <span><span className="num text-cream">{fmtInt(T.synced)}</span> of <span className="num">{fmtInt(T.expected)}</span> tracks synced{T.partial > 0 && <span className="text-amber"> · {T.partial} playlists only partly synced</span>}{T.unreadable > 0 && <span className="text-dust" title="Spotify closed its own playlists (Discover Weekly, editorial lists) to third-party apps in late 2024; their tracks can't be read."> · {T.unreadable} Spotify-made, unreadable</span>}</span>
      ...
      <div className="mt-6 grid items-start gap-6 lg:grid-cols-[1fr_1.4fr]">
              ...
              <li key={p.playlistId}>
                <button onClick={() => { setOpen(p.playlistId); setKindFilter('all'); }} className={`w-full py-2 text-left hover:text-amber ${open === p.playlistId ? 'text-amber' : ''}`}>
                  ...
                  <span className="mt-1 flex items-center gap-2">
                    ...
                    {p.affinity != null && <span title={`Affinity ${Math.round(p.affinity * 100)}% — ${p.affLoved} songs you love, ${p.affPlayed} you've played, ${p.affKnownArtist} unplayed by artists you know, of ${p.affItems}`} className={`num shrink-0 rounded-full px-1.5 text-[11px] ${p.affinity >= 0.6 ? 'bg-moss/20 text-moss' : p.affinity >= 0.35 ? 'bg-amber/15 text-amber' : 'bg-raised text-dust'}`}>{Math.round(p.affinity * 100)}%</span>}
        ...
        <Card title={cur ? cur.name : 'Pick a playlist'}
          subtitle={cur ? `${cur.affinity != null ? `${Math.round(cur.affinity * 100)}% your kind of playlist (${cur.affLoved} songs you love · ${cur.affPlayed} played · ${cur.affKnownArtist} unplayed by artists you know) · ` : ''}${cur.heard} of ${cur.synced} heard (${fmtPct(cur.completion)}) · ${fmtInt(cur.playsWithin)} plays · skips ${fmtPct(cur.skipRate)}${cur.lastPlayedWithin ? ` · last ${fmtDate(cur.lastPlayedWithin, { month: 'short', year: 'numeric' })}` : ''}${cur.partial ? ` · only ${cur.synced} of ${cur.trackCount} tracks synced` : ''}` : undefined}
